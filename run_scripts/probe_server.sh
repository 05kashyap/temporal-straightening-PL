...
CONTAINER_CONDA="${CONTAINER_CONDA:-/opt/conda-envs/ts}"   # the ts env prefix itself (as mpc_server.sh)
...
# --dry-run is forwarded into the container: the job then discovers the groups and
# prints the commands without running a single probe (rehearse before a 48 h submit).
DRY_RUN="${DRY_RUN:-0}"
[ "${1:-}" = "--dry-run" ] && DRY_RUN=1
...
# The container-side work is a real file (run_scripts/probe_container.sh) so it can be
# shell-checked and tested: it rebuilds PATH, treats conda as optional, resolves the
# interpreter by absolute path, and prints every step. `bash -c` (not `-lc`) because the
# image's login profile is one of the things that can drop /usr/bin from PATH, and nothing
# here needs it. Knobs travel as exported env vars + positional args, so apptainer's normal
# environment pass-through is the only channel.
export CONTAINER_CONDA CONDA_ENV REPO_IN_CONTAINER ENV_FILE DRY_RUN
"$APPTAINER_BIN" exec --fakeroot --nv "${BIND_ARGS[@]}" --overlay "$MOUNT" "$SIF" \
    bash -c "bash '$REPO_IN_CONTAINER/run_scripts/probe_container.sh' \
        '$CKPT_ROOT/test' '$DATA_ROOT' '$OUTDIR' '$ENVS' '$METHODS' '$SEED' '$TEST_FRAC' \
        '$RECIPES' '$ONLY' '$LIMIT_EPISODES' '$TMAX'"
